...
def _official_metadata_lookup(start: Path | None = None) -> tuple[dict[str, dict[str, Any]], Path | None]:
...
def _canonical_metadata_row(row: dict[str, Any], *, dsid: str, descriptor: str, generator: str, simulation: str, path: Path, root_meta: dict[str, Any], source_path: Path | None) -> dict[str, Any]:
...
def build_metadata_rows(inputs: Path) -> list[dict[str, Any]]:
    ...
    official, source_path = _official_metadata_lookup()
    for path in list_root_files(inputs / "MC"):
        ...
        if dsid in official:
            rows.append(_canonical_metadata_row(official[dsid], dsid=dsid, descriptor=descriptor, generator=generator, simulation=simulation, path=path, root_meta=meta, source_path=source_path))
        else:
